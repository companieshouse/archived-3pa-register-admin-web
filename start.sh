#!/bin/bash
#
# Start script for 3pa-register-admin-web
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/3pa-register-admin-web/env

    PORT="${3PA_REGISTER_API_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/3pa-register-admin-web.jar"